...
.graham-page .onlineStore { padding-top:16px !important; }
...
<main class="graham-page">
    ...
    @if(isset($grahamSubcategory) && $grahamSubcategory && $grahamSubcategory->banner_url)
        <section class="gehnawaSection p-0 position-relative">
            {{-- Desktop Video --}}
            @if(Str::endsWith($grahamSubcategory->banner_url, ['.mp4', '.webm', '.ogg']))
                <video 
                    autoplay 
                    loop 
                    muted 
                    ...
                    <source src="{{ asset($grahamSubcategory->banner_url) }}" type="video/{{ pathinfo($grahamSubcategory->banner_url, PATHINFO_EXTENSION) }}">
                    Your browser does not support the video tag.
                </video>
                {{-- Fallback image for desktop --}}
                <!-- <div class="video-fallback-desktop d-none d-md-block" style="display:none; width:100%; height:120vh; background-image:url('{{ asset($grahamSubcategory->banner_url) }}'); background-size:cover; background-position:center;"></div> -->
            @else
                {{-- Static image for desktop --}}
                <div class="d-none d-md-block" style="width:100%; height:120vh; background-image:url('{{ asset($grahamSubcategory->banner_url) }}'); background-size:cover; background-position:center;"></div>
            @endif

            {{-- Mobile Video (Dynamic based on subcategory) --}}
            @php
                $mobileVideo = null;
                $mobileVideoPath = 'assets/f_assets/image/watches mobile view/graham_mobile.mp4'; // Corrected path without assets/ prefix
                if ($grahamSubcategory->slug === 'graham') {
                    $mobileVideo = $mobileVideoPath;
                } else {
                    // fallback if no specific mobile version exists
                    $mobileVideo = $grahamSubcategory->banner_url;
                }
            @endphp
  ...
  z-index: 19999 !important;
      ...
      <div class="graham-filter-header navbar navbar-white filter">
        ...
        <div class="filter d-flex justify-content-end px-3">
            <button class="navbar-toggler border-0 text-black" type="button" data-bs-toggle="offcanvas" data-bs-target="#offcanvasGraham" aria-controls="offcanvasGraham" aria-label="Toggle navigation" style="position:static!important; margin:0!important;">
                <span class="navbar-toggler-icon"></span> SORT & FILTER
            </button>
        </div>

        <div class="container-fluid px-3">
        <div class="row onlineStore g-2 pt-3" id="grahamGrid">
        ...
        <div class="text-center py-4 graham-footer">
        @if($products->count() > 0)
            @php
    ...
    <div class="offcanvas offcanvas-end offcanvas-modern" tabindex="-1" id="offcanvasGraham" aria-labelledby="offcanvasGrahamLabel" data-bs-backdrop="true" data-bs-scroll="false">
        <div class="offcanvas-header">
            <span class="offcanvas-title" id="offcanvasGrahamLabel">SORT & FILTER</span>
            <button type="button" class="btn-close btn-close-black" data-bs-dismiss="offcanvas" aria-label="Close"></button>
        </div>
        <div class="offcanvas-body">
            <div>
                <div class="filter-section-title" onclick="toggleCategory('grahamSortList', this.querySelector('.category-toggle'))" style="font-size: 14px !important;">
                    Sort By <span class="category-toggle">+</span>
                </div>
                <ul class="sort-list" id="grahamSortList">
            ...
            <div>
                <div class="filter-section-title" onclick="toggleCategory('grahamGenderList', this.querySelector('.category-toggle'))" style="font-size: 14px !important;">Gender <span class="category-toggle">+</span></div>
                <ul class="category-list collapsible" id="grahamGenderList">
                    @php $selectedTags = collect(explode(',', request('tags', '')))->map(fn($s)=>trim($s)); @endphp
                    <li><input type="checkbox" class="form-check-input filter-tag-checkbox graham-filter" data-group="gender" value="mens" {{ $selectedTags->contains('mens') ? 'checked' : '' }} onclick="event.stopPropagation();"> <span class="subcat-label">Men's</span></li>
                    <li><input type="checkbox" class="form-check-input filter-tag-checkbox graham-filter" data-group="gender" value="ladies" {{ $selectedTags->contains('ladies') ? 'checked' : '' }} onclick="event.stopPropagation();"> <span class="subcat-label">Ladies</span></li>
                </ul>
            </div>
            <div class="mt-3">
                <div class="filter-section-title" onclick="toggleCategory('grahamSeriesList', this.querySelector('.category-toggle'))" style="font-size: 14px !important;">Series <span class="category-toggle">+</span></div>
                <ul class="category-list collapsible" id="grahamSeriesList">
                    @php $series = ['chronofighter-vintage','swordfish','chronofighter-superlight','fortress']; @endphp
                    @foreach($series as $s)
                        <li><input type="checkbox" class="form-check-input filter-tag-checkbox graham-filter" data-group="series" value="{{ $s }}" {{ $selectedTags->contains($s) ? 'checked' : '' }} onclick="event.stopPropagation();"> <span class="subcat-label">{{ ucwords(str_replace('-', ' ', $s)) }}</span></li>
                    @endforeach
                </ul>
            </div>
            <div class="mt-3">
    ...
    (function(){
        const offcanvas = document.getElementById('offcanvasGraham');
        function buildUrl() {
            ...
            const selected = Array.from(document.querySelectorAll('.graham-filter:checked')).map(i=>i.value);
        ...
        function fetchAndRender(url) {
            ...
            fetch(url.toString(), { headers: { 'X-Requested-With': 'XMLHttpRequest' }})
                ...
                .then(html => {
                    ...
                    const incomingGrid = doc.querySelector('#grahamGrid');
                    const grid = document.querySelector('#grahamGrid');
                    
                    if (incomingGrid && grid) {
                        grid.innerHTML = incomingGrid.innerHTML;
                    }
                    
                    const incomingFooter = doc.querySelector('.graham-footer');
                    const footer = document.querySelector('.graham-footer');
                    if (footer) {
        ...
        (function(){
            const sortList = document.getElementById('grahamSortList');
            if (!sortList) return;
        ...
        document.querySelectorAll('.graham-filter').forEach(cb => {
    ...
    document.addEventListener('DOMContentLoaded', function() {
        window.bindLoadMore = function bindLoadMore() {
            ...
            function getGrid(container) {
                return container.querySelector('#grahamGrid');
            }
            ...
            function appendIncomingItems(doc) {
                ...
                // Primary: take children of incoming #grahamGrid
                let nodesToAppend = [];
                const incomingGrid = getGrid(doc) || doc.querySelector('#grahamGrid');
                if (incomingGrid) {
            ...
            window.updateCounter = function updateCounter() {
                ...
                const counter = document.querySelector('.graham-footer .products-counter');
            ...
            btn.addEventListener('click', function() {
                ...
                fetch(url.toString(), { headers: { 'X-Requested-With': 'XMLHttpRequest' }, cache: 'no-cache' })
                    ...
                    .then(html => {
                        ...
                        if (appended === 0) {
                            const currentGrid = document.querySelector('#grahamGrid');
                            const incomingGrid2 = doc.querySelector('#grahamGrid');
