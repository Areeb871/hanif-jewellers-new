...
<main class="armand-page">
    ...
    @if(isset($armandNicoletSubcategory) && $armandNicoletSubcategory && $armandNicoletSubcategory->banner_url)
        <section class="gehnawaSection p-0 position-relative">
            {{-- Desktop Video --}}
            @if(Str::endsWith($armandNicoletSubcategory->banner_url, ['.mp4', '.webm', '.ogg']))
                <video 
                    autoplay 
                    loop 
                    muted 
                    ...
                    <source src="{{ asset($armandNicoletSubcategory->banner_url) }}" type="video/{{ pathinfo($armandNicoletSubcategory->banner_url, PATHINFO_EXTENSION) }}">
            ...
            @php
               ...
               $mobileVideoPath = 'assets/f_assets/image/watches mobile view/armond_niclet_mobile.mp4'; // Corrected path without assets/ prefix
                ...
                if ($armandNicoletSubcategory->slug === 'armand-nicolet') {
                    ...
                    $mobileVideo = $armandNicoletSubcategory->banner_url;
                }
            @endphp
...
.offcanvas-backdrop{
  z-index: 19999 !important;
        ...
        <div class="row onlineStore g-2 pt-3" id="armandNicoletGrid">
        ...
        <div class="text-center py-4 armand-nicolet-footer">
        @if($products->count() > 0)
            @php
    ...
    <div class="offcanvas offcanvas-end offcanvas-modern" tabindex="-1" id="offcanvasArmandNicolet" aria-labelledby="offcanvasArmandNicoletLabel" data-bs-backdrop="true" data-bs-scroll="false">
        <div class="offcanvas-header">
            <span class="offcanvas-title" id="offcanvasArmandNicoletLabel">SORT & FILTER</span>
            <button type="button" class="btn-close btn-close-black" data-bs-dismiss="offcanvas" aria-label="Close"></button>
        </div>
        <div class="offcanvas-body">
            <div>
                <div class="filter-section-title" onclick="toggleCategory('armandNicoletSortList', this.querySelector('.category-toggle'))" style="font-size: 14px !important;">
                    Sort By <span class="category-toggle">+</span>
                </div>
                <ul class="sort-list" id="armandNicoletSortList">
            ...
            <div>
                <div class="filter-section-title" onclick="toggleCategory('armandNicoletGenderList', this.querySelector('.category-toggle'))" style="font-size: 14px !important;">Gender <span class="category-toggle">+</span></div>
                <ul class="category-list collapsible" id="armandNicoletGenderList">
                    @php $selectedTags = collect(explode(',', request('tags', '')))->map(fn($s)=>trim($s)); @endphp
                    <li><input type="checkbox" class="form-check-input filter-tag-checkbox armand-nicolet-filter" data-group="gender" value="mens" {{ $selectedTags->contains('mens') ? 'checked' : '' }} onclick="event.stopPropagation();"> <span class="subcat-label">Men's</span></li>
                    <li><input type="checkbox" class="form-check-input filter-tag-checkbox armand-nicolet-filter" data-group="gender" value="ladies" {{ $selectedTags->contains('ladies') ? 'checked' : '' }} onclick="event.stopPropagation();"> <span class="subcat-label">Ladies</span></li>
                </ul>
            </div>
            <div class="mt-3">
    ...
    (function(){
        const offcanvas = document.getElementById('offcanvasArmandNicolet');
        function buildUrl() {
        ...
        function fetchAndRender(url) {
            ...
            fetch(url.toString(), { headers: { 'X-Requested-With': 'XMLHttpRequest' }})
                ...
                .then(html => {
                    ...
                    const incomingGrid = doc.querySelector('#armandNicoletGrid');
                    const grid = document.querySelector('#armandNicoletGrid');
                    
                    if (incomingGrid && grid) {
                        grid.innerHTML = incomingGrid.innerHTML;
                    }
                    
                    const incomingFooter = doc.querySelector('.armand-nicolet-footer');
                    const footer = document.querySelector('.armand-nicolet-footer');
                    if (footer) {
        ...
        (function(){
            const sortList = document.getElementById('armandNicoletSortList');
            if (!sortList) return;
        ...
        document.querySelectorAll('.armand-nicolet-filter').forEach(cb => {
    ...
    document.addEventListener('DOMContentLoaded', function() {
        window.bindLoadMore = function bindLoadMore() {
            ...
            function getGrid(container) {
                return container.querySelector('#armandNicoletGrid');
            }
            ...
            function appendIncomingItems(doc) {
                ...
                // Primary: take children of incoming #armandNicoletGrid
                let nodesToAppend = [];
                const incomingGrid = getGrid(doc) || doc.querySelector('#armandNicoletGrid');
                if (incomingGrid) {
            ...
            window.updateCounter = function updateCounter() {
                ...
                const counter = document.querySelector('.armand-nicolet-footer .products-counter');
            ...
            btn.addEventListener('click', function() {
                ...
                fetch(url.toString(), { headers: { 'X-Requested-With': 'XMLHttpRequest' }, cache: 'no-cache' })
                    ...
                    .then(html => {
                        ...
                        if (appended === 0) {
                            const currentGrid = document.querySelector('#armandNicoletGrid');
                            const incomingGrid2 = doc.querySelector('#armandNicoletGrid');
