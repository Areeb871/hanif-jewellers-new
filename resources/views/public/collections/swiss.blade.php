...
@section('content')
    @if(isset($swissMilitarySubcategory) && $swissMilitarySubcategory && $swissMilitarySubcategory->banner_url)
        <section class="gehnawaSection p-0 position-relative">
            {{-- Desktop Video --}}
            @if(Str::endsWith($swissMilitarySubcategory->banner_url, ['.mp4', '.webm', '.ogg']))
                <video 
                    autoplay 
                    loop 
                    muted 
                    ...
                    <source src="{{ asset($swissMilitarySubcategory->banner_url) }}" type="video/{{ pathinfo($swissMilitarySubcategory->banner_url, PATHINFO_EXTENSION) }}">
                    Your browser does not support the video tag.
                </video>
                {{-- Fallback image for desktop --}}
                <div class="video-fallback-desktop d-none d-md-block" style="display:none; width:100%; height:120vh; background-image:url('{{ asset($swissMilitarySubcategory->banner_url) }}'); background-size:cover; background-position:center;"></div>
            @else
                {{-- Static image for desktop --}}
                <div class="d-none d-md-block" style="width:100%; height:120vh; background-image:url('{{ asset($swissMilitarySubcategory->banner_url) }}'); background-size:cover; background-position:center;"></div>
            @endif
            ...
            @php
                ...
                if ($swissMilitarySubcategory->slug === 'swiss-military') {
                    ...
                    $mobileVideo = $swissMilitarySubcategory->banner_url;
                }
            @endphp
...
.offcanvas-backdrop{
  z-index: 19999 !important;
    ...
    SORT & FILTER
        ...
        <div class="row onlineStore g-2 pt-3" id="swissGrid">
        ...
        <div class="text-center py-5 swiss-footer">
            ...
            @php
                ...
                $totalFiltered = $totalFilteredProducts ?? $products->total();
                $totalShown = (($page - 1) * $perPage) + $currentPageProducts;
                $totalShown = min($totalShown, $totalFiltered);
            ...
            @endif
            @php
                $allProductsShown = $totalShown >= $totalFiltered;
    ...
    <div class="offcanvas offcanvas-end offcanvas-modern" tabindex="-1" id="offcanvasSwiss" aria-labelledby="offcanvasSwissLabel" data-bs-backdrop="true" data-bs-scroll="false">
        <div class="offcanvas-header">
            <span class="offcanvas-title" id="offcanvasSwissLabel">SORT & FILTER</span>
            <button type="button" class="btn-close btn-close-black" data-bs-dismiss="offcanvas" aria-label="Close"></button>
        </div>
        <div class="offcanvas-body">
            <div>
                <div class="filter-section-title" onclick="toggleCategory('swissSortList', this.querySelector('.category-toggle'))" style="font-size: 14px !important;">
                    Sort By <span class="category-toggle">+</span>
                </div>
                <ul class="sort-list" id="swissSortList">
            ...
            <div>
                <div class="filter-section-title" onclick="toggleCategory('swissGenderList', this.querySelector('.category-toggle'))" style="font-size: 14px !important;">Gender <span class="category-toggle">+</span></div>
                <ul class="category-list collapsible" id="swissGenderList">
                    @php $selectedTags = collect(explode(',', request('tags', '')))->map(fn($s)=>trim($s)); @endphp
                    <li><input type="checkbox" class="form-check-input filter-tag-checkbox swiss-filter" data-group="gender" value="mens" {{ $selectedTags->contains('mens') ? 'checked' : '' }} onclick="event.stopPropagation();"> <span class="subcat-label">Men's</span></li>
                    <li><input type="checkbox" class="form-check-input filter-tag-checkbox swiss-filter" data-group="gender" value="ladies" {{ $selectedTags->contains('ladies') ? 'checked' : '' }} onclick="event.stopPropagation();"> <span class="subcat-label">Ladies</span></li>
                </ul>
            </div>
            <div class="mt-3">
    ...
    (function(){
        const offcanvas = document.getElementById('offcanvasSwiss');
        function buildUrl() {
            const url = new URL(window.location.href);
            // Build unified tags param to match server-side filtering
            url.searchParams.delete('tags');
            url.searchParams.delete('gender');
            url.searchParams.delete('caseSize');
            url.searchParams.delete('movementType');
            url.searchParams.delete('caseType');
            const selected = Array.from(document.querySelectorAll('.swiss-filter:checked')).map(i=>i.value);
        ...
        function fetchAndRender(url) {
            ...
            fetch(url.toString(), { headers: { 'X-Requested-With': 'XMLHttpRequest' }})
                ...
                .then(html => {
                    ...
                    const incomingGrid = doc.querySelector('#swissGrid');
                    const grid = document.querySelector('#swissGrid');
                    
                    if (incomingGrid && grid) {
                        grid.innerHTML = incomingGrid.innerHTML;
                    }
                    
                    const incomingFooter = doc.querySelector('.swiss-footer');
                    const footer = document.querySelector('.swiss-footer');
                    if (footer) {
        ...
        (function(){
            const sortList = document.getElementById('swissSortList');
            if (!sortList) return;
        ...
        document.querySelectorAll('.swiss-filter').forEach(cb => {
    ...
    document.addEventListener('DOMContentLoaded', function() {
        window.bindLoadMore = function bindLoadMore() {
            ...
            function getGrid(container) {
                return container.querySelector('#swissGrid');
            }
            ...
            function appendIncomingItems(doc) {
                ...
                // Primary: take children of incoming #swissGrid
                let nodesToAppend = [];
                const incomingGrid = getGrid(doc) || doc.querySelector('#swissGrid');
                if (incomingGrid) {
            ...
            window.updateCounter = function updateCounter() {
                ...
                const counter = document.querySelector('.swiss-footer .products-counter');
            ...
            btn.addEventListener('click', function() {
                ...
                fetch(url.toString(), { headers: { 'X-Requested-With': 'XMLHttpRequest' }, cache: 'no-cache' })
                    ...
                    .then(html => {
                        ...
                        if (appended === 0) {
                            const currentGrid = document.querySelector('#swissGrid');
                            const incomingGrid2 = doc.querySelector('#swissGrid');
