...
Explore a world of limitless watchmaking creativity with ArtyA, the Geneva-based independent brand renowned for offering the most diverse range of timepieces globally.
    ...
    <div class="offcanvas offcanvas-end offcanvas-modern" tabindex="-1" id="offcanvasArtya" aria-labelledby="offcanvasArtyaLabel" data-bs-backdrop="true" data-bs-scroll="false">
        <div class="offcanvas-header">
            <span class="offcanvas-title" id="offcanvasArtyaLabel">SORT & FILTER</span>
            <button type="button" class="btn-close btn-close-black" data-bs-dismiss="offcanvas" aria-label="Close"></button>
        </div>
        <div class="offcanvas-body">
            <div>
                <div class="filter-section-title" onclick="toggleCategory('artyaSortList', this.querySelector('.category-toggle'))" style="font-size: 14px !important;">
                    Sort By <span class="category-toggle">+</span>
                </div>
                <ul class="sort-list" id="artyaSortList">
            ...
            <div>
                <div class="filter-section-title" onclick="toggleCategory('artyaGenderList', this.querySelector('.category-toggle'))" style="font-size: 14px !important;">Gender <span class="category-toggle">+</span></div>
                <ul class="category-list collapsible" id="artyaGenderList">
                    @php $selectedTags = collect(explode(',', request('tags', '')))->map(fn($s)=>trim($s)); @endphp
                    <li><input type="checkbox" class="form-check-input filter-tag-checkbox artya-filter" data-group="gender" value="mens" {{ $selectedTags->contains('mens') ? 'checked' : '' }} onclick="event.stopPropagation();"> <span class="subcat-label">Men's</span></li>
                    <li><input type="checkbox" class="form-check-input filter-tag-checkbox artya-filter" data-group="gender" value="ladies" {{ $selectedTags->contains('ladies') ? 'checked' : '' }} onclick="event.stopPropagation();"> <span class="subcat-label">Ladies</span></li>
                </ul>
            </div>
            <div class="mt-3">
    ...
    (function(){
        const offcanvas = document.getElementById('offcanvasArtya');
        function buildUrl() {
        ...
        function fetchAndRender(url) {
            ...
            fetch(url.toString(), { headers: { 'X-Requested-With': 'XMLHttpRequest' }})
                ...
                .then(html => {
                    ...
                    const incomingGrid = doc.querySelector('#artyaGrid');
                    const grid = document.querySelector('#artyaGrid');
                    
                    if (incomingGrid && grid) {
                        grid.innerHTML = incomingGrid.innerHTML;
                    }
                    
                    const incomingFooter = doc.querySelector('.artya-footer');
                    const footer = document.querySelector('.artya-footer');
                    if (footer) {
        ...
        (function(){
            const sortList = document.getElementById('artyaSortList');
            if (!sortList) return;
        ...
        document.querySelectorAll('.artya-filter').forEach(cb => {
    ...
    document.addEventListener('DOMContentLoaded', function() {
        window.bindLoadMore = function bindLoadMore() {
            ...
            function getGrid(container) {
                return container.querySelector('#artyaGrid');
            }
            ...
            function appendIncomingItems(doc) {
                ...
                // Primary: take children of incoming #artyaGrid
                let nodesToAppend = [];
                const incomingGrid = getGrid(doc) || doc.querySelector('#artyaGrid');
                if (incomingGrid) {
            ...
            window.updateCounter = function updateCounter() {
                ...
                const counter = document.querySelector('.artya-footer .products-counter');
            ...
            btn.addEventListener('click', function() {
                ...
                fetch(url.toString(), { headers: { 'X-Requested-With': 'XMLHttpRequest' }, cache: 'no-cache' })
                    ...
                    .then(html => {
                        ...
                        if (appended === 0) {
                            const currentGrid = document.querySelector('#artyaGrid');
                            const incomingGrid2 = doc.querySelector('#artyaGrid');
