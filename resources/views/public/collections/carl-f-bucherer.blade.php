...
@section('content')
    @if(isset($carlFBuchererSubcategory) && $carlFBuchererSubcategory && $carlFBuchererSubcategory->banner_url)
        <section class="gehnawaSection p-0 position-relative">
            {{-- Desktop Video --}}
            @if(Str::endsWith($carlFBuchererSubcategory->banner_url, ['.mp4', '.webm', '.ogg']))
                <video 
                    autoplay 
                    loop 
                    muted 
                    ...
                    <source src="{{ asset($carlFBuchererSubcategory->banner_url) }}" type="video/{{ pathinfo($carlFBuchererSubcategory->banner_url, PATHINFO_EXTENSION) }}">
                    Your browser does not support the video tag.
                </video>
                {{-- Fallback image for desktop --}}
                <div class="video-fallback-desktop d-none d-md-block" style="display:none; width:100%; height:120vh; background-image:url('{{ asset($carlFBuchererSubcategory->banner_url) }}'); background-size:cover; background-position:center;"></div>
            @else
                {{-- Static image for desktop --}}
                <div class="d-none d-md-block" style="width:100%; height:120vh; background-image:url('{{ asset($carlFBuchererSubcategory->banner_url) }}'); background-size:cover; background-position:center;"></div>
            @endif

            {{-- Mobile Video (Dynamic based on subcategory) --}}
            @php
                $mobileVideo = null;
                $mobileVideoPath = 'assets/f_assets/image/watches mobile view/carl_bukherer.mp4'; // Corrected path without assets/ prefix

                if ($carlFBuchererSubcategory->slug === 'carl-f-bucherer') {
                    $mobileVideo = $mobileVideoPath;
                } else {
                    // fallback if no specific mobile version exists
                    $mobileVideo = $carlFBuchererSubcategory->banner_url;
                }
            @endphp
    ...
    <section class="py-4">
        <style>
            ...
            .offcanvas-modern .offcanvas-body { background: rgb(255, 255, 255); padding: 1rem; visibility: visible !important; opacity: 1 !important; }
            .offcanvas-modern.show .offcanvas-body { display: block !important; visibility: visible !important; }
            #offcanvasCarlFBucherer .offcanvas-body > div { display: block !important; visibility: visible !important; }
        ...
        <div class="navbar navbar-white align-items-center filter position-relative justify-content-center">
            <div class="brand-logo-wrapper w-70 my-3 text-center">
                <img src="{{ asset('assets/f_assets/image/watch logo/CFB.png') }}" alt="Carl F. Bucherer logo" class="brand-logo">
            </div>
            <button class="navbar-toggler border-0 text-black position-absolute end-0" type="button" data-bs-toggle="offcanvas" data-bs-target="#offcanvasCarlFBucherer" aria-controls="offcanvasCarlFBucherer" aria-label="Toggle navigation">
                <span class="navbar-toggler-icon"></span> SORT & FILTER
            </button>
        </div>

        <div class="container-fluid px-3">
        <div class="row onlineStore g-2 pt-3" id="carlFBuchererGrid">
        ...
        <div class="text-center py-4 carl-f-bucherer-footer">
        @if($products->count() > 0)
            @php
    ...
    <div class="offcanvas offcanvas-end offcanvas-modern" tabindex="-1" id="offcanvasCarlFBucherer" aria-labelledby="offcanvasCarlFBuchererLabel" data-bs-backdrop="false" data-bs-scroll="true">
        <div class="offcanvas-header">
            <span class="offcanvas-title" id="offcanvasCarlFBuchererLabel">SORT & FILTER</span>
            <button type="button" class="btn-close btn-close-black" data-bs-dismiss="offcanvas" aria-label="Close"></button>
        </div>
        <div class="offcanvas-body">
            <div>
                <div class="filter-section-title" onclick="toggleCategory('carlFBuchererSortList', this.querySelector('.category-toggle'))" style="font-size: 14px !important;">
                    SORT BY <span class="category-toggle">−</span>
                </div>
                <ul class="sort-list show" id="carlFBuchererSortList">
            ...
            <div>
                <div class="filter-section-title" onclick="toggleCategory('carlFBuchererGenderList', this.querySelector('.category-toggle'))" style="font-size: 14px !important;">GENDER <span class="category-toggle">+</span></div>
                <ul class="category-list collapsible" id="carlFBuchererGenderList">
                    @php $selectedTags = collect(explode(',', request('tags', '')))->map(fn($s)=>trim($s)); @endphp
                    <li><input type="checkbox" class="form-check-input filter-tag-checkbox carl-f-bucherer-filter" data-group="gender" value="mens" {{ $selectedTags->contains('mens') ? 'checked' : '' }} onclick="event.stopPropagation();"> <span class="subcat-label">Men's</span></li>
                    <li><input type="checkbox" class="form-check-input filter-tag-checkbox carl-f-bucherer-filter" data-group="gender" value="ladies" {{ $selectedTags->contains('ladies') ? 'checked' : '' }} onclick="event.stopPropagation();"> <span class="subcat-label">Ladies</span></li>
                </ul>
            </div>
            <div class="mt-3">
                <div class="filter-section-title" onclick="toggleCategory('carlFBuchererSeriesList', this.querySelector('.category-toggle'))" style="font-size: 14px !important;">SERIES <span class="category-toggle">+</span></div>
                <ul class="category-list collapsible" id="carlFBuchererSeriesList">
                    @php $series = ['manero', 'patravi', 'heritage', 'adamavi', 'pathos']; @endphp
                    @foreach($series as $s)
                        <li><input type="checkbox" class="form-check-input filter-tag-checkbox carl-f-bucherer-filter" data-group="series" value="{{ $s }}" {{ $selectedTags->contains($s) ? 'checked' : '' }} onclick="event.stopPropagation();"> <span class="subcat-label">{{ ucfirst($s) }}</span></li>
                    @endforeach
                </ul>
            </div>
        </div>
    ...
    (function(){
        const offcanvas = document.getElementById('offcanvasCarlFBucherer');
        function buildUrl() {
            ...
            const selected = Array.from(document.querySelectorAll('.carl-f-bucherer-filter:checked')).map(i=>i.value);
        ...
        function fetchAndRender(url) {
            ...
            fetch(url.toString(), { headers: { 'X-Requested-With': 'XMLHttpRequest' }})
                ...
                .then(html => {
                    ...
                    const incomingGrid = doc.querySelector('#carlFBuchererGrid');
                    const grid = document.querySelector('#carlFBuchererGrid');
                    
                    if (incomingGrid && grid) {
                        grid.innerHTML = incomingGrid.innerHTML;
                    }
                    
                    const incomingFooter = doc.querySelector('.carl-f-bucherer-footer');
                    const footer = document.querySelector('.carl-f-bucherer-footer');
                    if (footer) {
                    ...
                    // Re-attach sort handlers after AJAX update
                    attachSortHandlers();
        ...
        function attachSortHandlers() {
            const sortList = document.getElementById('carlFBuchererSortList');
            if (!sortList) return;
        ...
        // Attach sort handlers on page load
        attachSortHandlers();

        // Checkbox immediate apply like online store
        document.querySelectorAll('.carl-f-bucherer-filter').forEach(cb => {
    ...
    document.addEventListener('DOMContentLoaded', function() {
        window.bindLoadMore = function bindLoadMore() {
            ...
            function getGrid(container) {
                return container.querySelector('#carlFBuchererGrid');
            }
            ...
            function appendIncomingItems(doc) {
                ...
                // Primary: take children of incoming #carlFBuchererGrid
                let nodesToAppend = [];
                const incomingGrid = getGrid(doc) || doc.querySelector('#carlFBuchererGrid');
                if (incomingGrid) {
            ...
            window.updateCounter = function updateCounter() {
                ...
                const counter = document.querySelector('.carl-f-bucherer-footer .products-counter');
            ...
            btn.addEventListener('click', function() {
                ...
                fetch(url.toString(), { headers: { 'X-Requested-With': 'XMLHttpRequest' }, cache: 'no-cache' })
                    ...
                    .then(html => {
                        ...
                        if (appended === 0) {
                            const currentGrid = document.querySelector('#carlFBuchererGrid');
                            const incomingGrid2 = doc.querySelector('#carlFBuchererGrid');
