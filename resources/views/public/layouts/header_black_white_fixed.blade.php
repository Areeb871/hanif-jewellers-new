...
<div class="header-spacer"></div>
  ...
  @endphp
    ...
    <style>
        ...
        body.nav-search-modal-open .modal-backdrop.show {
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  function setHeaderBackground() {
    ...
    const heroPassed = hero ? hero.getBoundingClientRect().bottom <= headerHeight : window.scrollY > 80;

    if (desktopHeader) desktopHeader.classList.toggle('hero-passed', heroPassed);
    if (mobileHeader) mobileHeader.classList.toggle('hero-passed', heroPassed);
  }
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const renderResults = (term = '') => {
    ...
    if (searchCount) searchCount.textContent = `Results (${matches.length})`;
  ...
  if (searchModalEl) {
    searchModalEl.addEventListener('show.bs.modal', () => {
      document.body.classList.add('nav-search-modal-open');
    });
    ...
    searchModalEl.addEventListener('hidden.bs.modal', () => {
      document.body.classList.remove('nav-search-modal-open');
    });
  }
});
</script>
